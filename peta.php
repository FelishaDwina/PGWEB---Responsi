...
<?php
// Fetch all pelabuhan data for JavaScript
$query = mysqli_query($koneksi, "SELECT id, Nama, Jenis, Lokasi, Latitude, Longitude, Deskripsi, Gambar FROM pelabuhan");
$pelabuhan_data = [];
while ($data = mysqli_fetch_assoc($query)) {
    $pelabuhan_data[] = $data;
}
$pelabuhan_json = json_encode($pelabuhan_data);
?>
...
<script>
    ...
    var allPelabuhanData = <?= $pelabuhan_json; ?>; // PHP data passed to JS
    ...
    function createMarker(data) {
        ...
        popupContent += "<a href='detail.php?id=" + data.id + "' class='btn btn-info btn-xs mt-2'><i class='bi bi-info-circle me-1'></i> Detail</a>";
        popupContent += "</div>";
